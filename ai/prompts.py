...
- Интернет/IDE: НЕТ.
- Включать короткие примерные код/псевдокод: ДА (короткие фрагменты).
- Для senior/lead включать системный дизайн: ДА.
- Длина эталонных ответов: короткая (5–12 слов).

СТРОКОВЫЕ ОГРАНИЧЕНИЯ И ПРАВИЛА:
- Длина плана технического собеседования 400-600 слов.
- Язык — русский.
...
1) КОНТЕКСТ (1–3 строки)
   - Коротко: стадия, длительность, уровень позиции (если в доках; иначе «нетданных»). ℹ️

2) ТАЙМИНГ (минуты, суммарно = 60)
   - Короткий тайминг по блокам (примерно): ввод 5 мин, профиль/опыт 15 мин, профильные тех. вопросы 30 мин, вопросы кандидата 10 мин. ✅

3) ТЕХНИЧЕСКИЕ ВОПРОСЫ (10–15)
   Для каждого — строго:
   - Вопрос (1 строка).  
   - Уровень сложности: [L/M/H].  
   - 🔴 Красный флаг (1–2 слова).  
   - ✅ Эталонный ответ (5–12 слов).  
   - Follow-up (1 строка), если уместно (например: «Приведите пример»).  
   Задавать вопросы строго по требованиям вакансии; не уходить в нерелевантную теорию.

4) ПРИМЕРНЫЕ КОРОТКИЕ КОД/ПСЕВДОКОД-ФРАГМЕНТЫ (только если релевантно)
   - Для 1–3 вопросов дать 1–2 строки псевдокода или ключевые команды/конструкции.
   - Не писать длинных реализаций — только демонстрация подхода.

5) СИСТЕМНЫЙ ДИЗАЙН (только если уровень senior/lead или явно требуется)
   - Одно короткое задание (1–2 строки).
   - 3 ключевых аспекта для оценки (по пунктам).
   - 🔴 Красные флаги (1–2 слова).
   - ✅ Идеальные упоминания/решения (коротко).

6) ВОПРОСЫ ОТ КАНДИДАТА ТЕХНИЧЕСКОМУ ИНТЕРВЬЮЕРУ (10–15)
   Для каждого:
   - Вопрос (1 строка).
   - 🔴 Красный флаг (ответ интервьюера, который настораживает — 1–2 слова).
   - ✅ Надёжный ответ интервьюера (коротко).
...
7) КРАТКАЯ ОЦЕНОЧНАЯ РУБРИКА (2–4 строки)
   - Предложить шкалу (например 0–4) и минимальный порог.
   - Указать, что считать «красным итогом» (одно-два слова).

8) ПОДСКАЗКИ ДЛЯ КАНДИДАТА (3–6 пунктов)
   - Каждый 3–8 слов: как отвечать на красные флаги, поведению в диалоге, распределению времени.

9) ФОРМАТ ВЫВОДА
   - Короткие буллеты, минимальный текст.
   - Все «🔴 красные флаги» — только слова/словосочетания.
   - Все «✅ эталонные ответы» — 5–12 слов.
   - Пометки «нетданных» или «несоответствие» где нужно.

ДОП. ПРИМЕЧАНИЯ ДЛЯ МОДЕЛИ:
