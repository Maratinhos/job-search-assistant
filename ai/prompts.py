...
Ограничения:
1. Длина плана созвона 300-400 слов (без учета эмодзи).
2. Не должно содержать контактов, персональных данных.
...
1. **Краткое самопрезентование кандидата** — 1 абзац: 2-3 предложения. Фокус на релевантном опыте/ключевом результате. (рекоменд. 1–2 мин)
2. **Вопросы от HR (8–10 вопросов)**. Для каждого вопроса:
   - 🔴 *Красный флаг* — ключевые слова/фразы, кратко, в одну строку.
   - ✅ *Пример корректного ответа* — краткое предложение.
   HR-вопросы — только поверхностные (цель подтвердить пункты резюме), без глубоких технических деталей.
3. **Вопросы кандидата к HR (8–10 вопросов)**. Включить обязательно вопросы про: 
   - позиция новая или нет; 
   - вилка (зарплатная); 
   - сколько времени занимает весь процесс найма; 
   - когда будет фидбек по текущему созвону.
   Для каждого вопроса:
   - 🔴 *Красный флаг* - от HR (ключевые слова/фразы) — что считать тревожным/уклончивым ответом;
   - ✅ *Пример надёжного ответа HR* — краткое предложение;
...
4. Все вопросы должны быть пронумерованы и разбиты по блокам для легкой и быстрой навигации по ним.

Доп. правила:
- Используй только информацию из резюме и вакансии. Если какой-то пункт в них отсутствует — помечай это кратко («нет данных») там, где логично.
...
Примеры вопросов:
1. Новая ли это позиция? 
   🔴 Неизвестно 
   ✅ Замещение
   📌 Скрипт-добивка: «У кого-то есть ответ на этот вопрос?»

2. Когда можно ожидать фидбек по текущему созвону?   
   🔴 Неизвестно 
   ✅ 2-3 рабочих дня
   📌 Скрипт-добивка: «Будет ли вообще фидбек по текущему созвону?»
"""

GENERATE_TECH_INTERVIEW_PLAN_PROMPT_CORE = """
Задача: на основе исключительно приложенных Резюме и Вакансии составить подробный план теоретической части технического собеседования (диалог, вопросы). 
НЕ использовать других источников. Тон — подстраиваться под тон вакансии.

КОНФИГУРАЦИЯ (фиксирована):
- Стадия: техническое собеседование (теоретическая часть).
- Длительность: 60 минут.
- Практические/кодовые задачи: НЕТ.
- Интернет/IDE: НЕТ.
- Для senior/lead включать системный дизайн: ДА.

Ограничения:
- Длина плана технического собеседования 500-600 слов (без учета эмодзи).
- Язык — русский.
- Никаких контактов или персональных данных.
- Ноль воды — только факты, короткие формулировки.
- Эмодзи: ✅ 🔴 ℹ️ (1–2 на пункт для визуализации).
- Никаких призывов к действию.
- Использовать только данные из резюме и вакансии. Если данные отсутствуют — пометить «нет данных». Если есть противоречие — пометить «несоответствие». Не придумывать технологий/проектов.

ТРЕБУЕМАЯ СТРУКТУРА ВЫХОДА (в этом порядке):

1. Технические вопросы (10–15)
   Для каждого — строго:
   - Вопрос.
   - 🔴 *Красный флаг* — 1–3 слова/словосочетания.
   - ✅ *Пример корректного ответа* — 2-3 слова.
   - Follow-up, если уместно (например: «Приведите пример»).  
   Задавать вопросы строго по требованиям вакансии; не уходить в нерелевантную теорию.

2. Системный дизайн (только если уровень senior/lead или явно требуется). Не отображать этот блок, если он не требуется.
   - Одно короткое задание.
   - 3 ключевых аспекта для оценки (по пунктам).
   - 🔴 *Красный флаг* — 1–3 слова/словосочетания.
   - ✅ *Пример корректного ответа* — коротко.

3. Вопросы от кандидата техническому интервьюеру (10-15)
   Для каждого:
   - Вопрос.
   - 🔴 *Красный флаг* — 1–3 слова/словосочетания.
   - ✅ *Пример корректного ответа* — 2-3 слова.
   - Если ответ может быть уклончивым — сразу давать скрипт-добивку (одно предложение, напр.: «Конкретно: сколько сервисов и средний трафик?»).
   Обязательные включения: позиция новая/замещение; когда будет фидбек по текущему интервью; как будет выглядеть полный рабочий день/неделя/месяц.

4. Все пункты и подппункты списка должны быть пронумерованы и разбиты по блокам для легкой и быстрой навигации по ним.

ДОП. ПРИМЕЧАНИЯ:
- Если вакансия указывает конкретный стек — задавать вопросы в этом стеке.
- Не включать практические задания/тесты (пользователь запретил).
- Для senior/lead — включать trade-offs в системном дизайне.
- Везде избегать личных данных и контактов.

Примеры вопросов:
1. Какой стек технологий основной? 
   🔴 Неизвестно/выбираем/меняется
   ✅ SQL, Power BI, GCP
   📌 Скрипт-добивка: «Кто этим управляет или влияет на это?»

2. Как будет выглядеть типичный рабочий день?   
   🔴 Неизвестно/решим/обсудим
   ✅ Участие в постановке задач, анализ данных, разработка дашбордов
   📌 Скрипт-добивка: «Будет ли вообще фидбек по текущему созвону?»
"""
